...
class ExerciseActivityDetailScreen extends StatelessWidget {
  final InsightController controller = Get.find();

  ExerciseActivityDetailScreen({super.key});
            ...
            child: Padding(
              ...
              child: Column(
                ...
                children: [
                  Column(
                    children: [
                      Image.asset(kExercise1Image,height: 157,width: 157,),
                  ...
                  SizedBox(height: getHeight(24),),
                  Text("Why Exercise?",style: AppStyles.primaryTextStyle().copyWith(fontSize: 20,fontWeight: FontWeight.w700,color: kRedColor),),
                  SizedBox(height: getHeight(16),),
                  Text("Inadequate amounts of physical exercise promotes ageing and contributes to various major diseases: heart disease, hypertension, stroke, diabetes, cancer, and Alzheimer's. Sedentary individuals have a 52% higher mortality rate than active individuals. "
                      "\n\nPhysical exercise also enhances cognitive functions such as learning and memory, as well as improving emotional regulation. "
                      "\n\nPeople are aware of the importance of exercise for health but most do not get enough. This is due to our survival instinct to conserve energy unless necessary, and since there is no-longer a necessity to be active for survival in the modern world, deliberate exercise becomes critical.",
                    style: AppStyles.blackTextStyle().copyWith(fontSize: 16,fontWeight: FontWeight.w500,),),
                  SizedBox(height: getHeight(24),),
                  Text("Exercise habits",style: AppStyles.primaryTextStyle().copyWith(fontSize: 20,fontWeight: FontWeight.w700,color: kRedColor),),
                  SizedBox(height: getHeight(16),),
                  Obx(() => ListView.builder(
                    padding: EdgeInsetsDirectional.all(0),
                    shrinkWrap: true,
                    itemCount: controller.exerciseTips.length,
                    physics: NeverScrollableScrollPhysics(),
                    itemBuilder: (context, index) {
                      final data = controller.exerciseTips[index];
                      return GestureDetector(
                        onTap: (){
                          if(index == 0) {
                            Get.toNamed(kFirstExcHabitDetailRoute);
                          } else if(index == 1){
                            Get.toNamed(kSecondExcHabitDetailRoute);
                          } else if(index == 2){
                            Get.toNamed(kThirdExcHabitDetailRoute);
                          } else if(index == 3){
                            Get.toNamed(kFourthExcHabitDetailRoute);
                          } else if(index == 4){
                            Get.toNamed(kFifthExcHabitDetailRoute);
                          }
                        },
                        child: Padding(
